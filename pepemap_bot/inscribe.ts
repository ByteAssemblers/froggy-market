...
const PEPECOIN_RPC_URL = "http://172.16.11.131:33873";
const PEPECOIN_RPC_USER = "luckylucker";
const PEPECOIN_RPC_PASSWORD = "Passkey123";
...
export async function buildCommitPsbtMulti({
  ...
  if (totalIn < outputsSum + feeEstimate) {
    throw new Error(
      `Insufficient funds for commit: need at least ${
        outputsSum + feeEstimate
      }, have ${totalIn}.`
    );
  }
  ...
  if (totalIn < outputsSum + fee) {
    ...
    throw new Error(
      `Insufficient funds after fee calc: need ${
        outputsSum + fee
      }, have ${totalIn} (short by ${shortfall}).`
    );
  }
...
async function callPepecoinRpc(
  ...
  if (PEPECOIN_RPC_USER) {
    config.auth = {
      username: PEPECOIN_RPC_USER,
      password: PEPECOIN_RPC_PASSWORD || "",
    };
  }

  console.log(`🔄 Preparing RPC call to: ${PEPECOIN_RPC_URL}`);
...
// Function to broadcast transaction via Pepecoin RPC
...
export async function broadcastRawTxCore(hex: any) {
  ...
  // Broadcasting the raw transaction via RPC
  const txid = await broadcastPepecoinTransaction(bodyHex, {
    allowHighFees: true,
  });
  return txid;
}
...
export async function waitForRawTx(
  ...
  throw new Error(
    `Raw tx not available for ${txid} after ${timeoutMs}ms. Last error: ${String(
      lastErr
    )}`
  );
}
